-- tsdb_008_viewer_daily.sql
-- Daily viewer device/OS/browser/country aggregates for the User & Device
-- dashboard. Fed by the mesh from BigQuery viewer_logs.viewer_requests_native
-- (the CDN viewer-request log, which has user_agent + country but no user_id).
-- The mesh aggregates each day in BigQuery and upserts the small result here.
-- Run on the TSDB instance via psql.

create table if not exists public.viewer_daily (
  day                date not null,
  ...
  primary key (day, device_type, os, browser, country)
);

-- Re-create get_firebase_analytics so devices / OS / browsers / geographic
-- come from viewer_daily (real CDN data) instead of content_sessions (which
-- never carries device/geo fields). Everything else stays on content_sessions.
...
begin
  ...
  vd_total as (
    select coalesce(sum(requests), 0) as n from public.viewer_daily where day between p_start and p_end
  ),
  devices as (
    select coalesce(device_type, 'Unknown') as device,
           sum(distinct_sessions)::bigint as users,
           sum(requests)::bigint as sessions,
           round(sum(requests) * 100.0 / nullif((select n from vd_total), 0), 2) as percentage
  ...
  devices_arr as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'avgSessionDuration', 0, 'percentage', percentage)
      order by users desc), '[]'::jsonb) as val from devices
  ),
  os_br as (
    select coalesce(os, 'Unknown') as os,
           ...
           round(sum(requests) * 100.0 / nullif((select n from vd_total), 0), 2) as percentage
    from public.viewer_daily
    where day between p_start and p_end and os is not null
    group by 1
    order by users desc
    limit 10
  ),
  os_arr as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'avgSessionDuration', 0, 'percentage', percentage)
      order by users desc), '[]'::jsonb) as val from os_br
  ),
  browser_br as (
    select coalesce(browser, 'Unknown') as browser,
           ...
           round(sum(requests) * 100.0 / nullif((select n from vd_total), 0), 2) as percentage
    from public.viewer_daily
    where day between p_start and p_end and browser is not null
    group by 1
    order by users desc
    limit 10
  ),
  browser_arr as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'avgSessionDuration', 0, 'percentage', percentage)
      order by users desc), '[]'::jsonb) as val from browser_br
  ),
  geo_br as (
    select coalesce(country, 'Unknown') as country,
           ...
           round(sum(requests) * 100.0 / nullif((select n from vd_total), 0), 2) as percentage
    from public.viewer_daily
    where day between p_start and p_end and country is not null
    group by 1
    order by users desc
    limit 15
  ),
